...
import { createOpsDb, runRegenerateDocumentsAction } from "@/lib/ops/opsActions";
...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json().catch(() => ({}))) as { booking_id?: string; confirm?: boolean };
    ...
    const result = await runRegenerateDocumentsAction(db, body.booking_id ?? "", {
      userId: auth.userId,
      role: auth.role,
    ...
    return routeError(500, "Failed to run regenerate documents action");
  }
}
