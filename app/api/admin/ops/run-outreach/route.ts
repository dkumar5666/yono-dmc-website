...
import { createOpsDb, runOutreachAction } from "@/lib/ops/opsActions";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await runOutreachAction(req, db, {
      userId: auth.userId,
      role: auth.role,
    ...
    return routeError(500, "Failed to run outreach action");
  }
}
