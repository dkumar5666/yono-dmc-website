...
import { ClipboardList, FileText, LayoutDashboard, PlusSquare } from "lucide-react";

const AGENT_NAV = [
  { href: "/agent/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/agent/leads", label: "Leads", icon: ClipboardList },
  { href: "/agent/leads/new", label: "Request Quote", icon: PlusSquare },
  { href: "/agent/quotes", label: "Quotations", icon: FileText },
  { href: "/agent/bookings", label: "Bookings", icon: ClipboardList },
];

export default function AgentLayoutShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  if (pathname === "/agent/login" || pathname === "/agent/signup") return <>{children}</>;
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[1400px]">
        <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white/80 backdrop-blur lg:flex lg:flex-col">
          <div className="border-b border-slate-200 px-5 py-4">
            <Link href="/agent/dashboard" className="flex items-center gap-3 rounded-xl p-2 hover:bg-slate-50">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">B2B Agent Portal</p>
              </div>
            </Link>
          </div>
