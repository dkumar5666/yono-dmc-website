      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {destinations.map((destination) => {
            const country = normalizeDestinationCountry(destination.country ?? destination.name);
        ...
        <div className="bg-teal-500 text-white rounded-2xl p-8 text-center">
          ...
          <p className="text-xl mb-6">
            Tell us where you want to go and we&apos;ll create a custom package for you
          </p>
          <WhatsAppButton text="Get Custom Package" className="bg-white text-teal-600 hover:bg-gray-100" />
        </div>
      </section>
    </div>
  );
