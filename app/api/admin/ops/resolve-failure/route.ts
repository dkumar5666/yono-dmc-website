...
import { createOpsDb, runResolveFailureAction } from "@/lib/ops/opsActions";
...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json().catch(() => ({}))) as { failure_id?: string; confirm?: boolean };
    ...
    const result = await runResolveFailureAction(db, body.failure_id ?? "", {
      userId: auth.userId,
      role: auth.role,
    ...
    return routeError(500, "Failed to resolve automation failure");
  }
}
