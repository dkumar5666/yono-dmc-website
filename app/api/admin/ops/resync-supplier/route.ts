...
import { createOpsDb, runResyncSupplierAction } from "@/lib/ops/opsActions";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await runResyncSupplierAction(db, body.booking_id ?? "", {
      userId: auth.userId,
      role: auth.role,
    ...
    return routeError(500, "Failed to run supplier resync action");
  }
}
