...
export async function POST(req: Request, { params }: { params: Params | Promise<Params> }) {
  ...
  await updateSupplierServiceStatus(context, "completed");
  await updateBookingSupplierStatus(context, "completed");

  const message = note || "Supplier marked service completed from portal";
